function [M mesh] = BuildLossMatrix(mesh, dim, CMFD, fine)
        ...
        for surf = 1:6          % 1 = -x; 2 = +x
            ...
            if (neigh == -1) %boundary
                ...
                if CMFD == true
                   ...
                   if (surf == 1 || surf == 2)
                   dhat = -(fine.Jsurf(i3,j3,k3,g,surf) - side *  mesh.dtilde(i,j,k,g,surf)* ...
                ...
                if CMFD == true
                   %calculate dhat
                    ...
                    if (xyz == 1 && side == -1)
                        ...
                        i4 = (i+side)*mesh.gridreduce(1);
                    elseif (xyz == 2 && side == -1)
                        j3 = (j+side)*mesh.gridreduce(2) + 1;
                        j4 = (j+side)*mesh.gridreduce(2);
                    elseif (xyz == 3 && side == -1) 
                        k3 = (k+side)*mesh.gridreduce(3) + 1;
                        k4 = (k+side)*mesh.gridreduce(3);  	
                    end
                   if (dim < 3)
                       k3 = k;
                   end
                   ...
                   current = fine.Jsurf(i3,j3,k3,g,surf);% +...
                       %fine.Jsurf(i4,j4,k4,g,surf2);
                   if (surf == 1 || surf == 2)                  
                   dhat = -(current + side * mesh.dtilde(i,j,k,g,surf) * ...
                       (-mesh.phi(i,j,k,g) + mesh.phi(i2,j2,k2,g))...
                       )/ (mesh.phi(i,j,k,g) + ...
                       mesh.phi(i2,j2,k2,g));
                   end
                   if (i == 2 && g == 1)
                       %phi1 = mesh.phi(i,j,k,g)
                       %phi2 = mesh.phi(i2,j2,k2,g)
                       %dhat
                       %mesh.dtilde(i,j,k,g,surf)
                       %current
                       %dhatfinal = 4.729087906933896e-004;                       
                   end
                end
            ...
            %mesh.Leakage(i,j,k,g,surf) = side * mesh.dtilde(i,j,k,g,surf) + dhat;
